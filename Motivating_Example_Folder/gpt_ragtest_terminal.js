//Gpt version of the contract labeling script.
//To run the motivatinng example use this link: https://stackoverflow.com/questions/47665391
//Generated Buggy and Fixed contract files may need refinement. You may execute the refined scripts:GPT_post_47665391_BUGGY.py and GPT_post_47665391_FIXED.py
...
function extractPyContractOutput(content) {
  ...
  // Back-compat (old single block & headings)
...
// ---- NEW: preserve code fences & formatting (HTML → Markdown) ----
function htmlToMarkdown(html) {
  let s = html || "";

  // Block code → fenced code with python hint
...
// ---- UPDATED: include title, preserve code blocks, optional all answers ----
async function fetchPostContent(postURL) {
  ...
  // Fetch question (HTML body + title)
  ...
  // Fetch answers and pick ONLY the accepted one
...
async function main() {
  ...
  // embed + retrieve few-shot examples (now includes title)
  const topK = context.embeddedExamples.slice(0, 5); // or however many you want
  ...
  if (SHOW_PY_PROMPT) {
    ...
    console.log("\n(ℹ️ Set SHOW_PY_PROMPT=1 to print the full Py prompt)\n");
  }
